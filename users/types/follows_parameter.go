...
func (p *FollowsFollowingGetParams) ResolveEndpoint(endpointBase string) string {
	...
	query := url.Values{}
	return endpoint + resolveFollowingQuery(query, p.MaxResult, p.PaginationToken, p.Expansions, p.TweetFields, p.UserFields)
}
...
func resolveFollowingQuery(q url.Values, max FollowsMaxResult, paginationToken string, expansions, tweetFields, userFields []string) string {
	if max.Valid() {
